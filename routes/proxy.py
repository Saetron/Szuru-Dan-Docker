    ...
    try:
        ...
        if response.status_code in [200, 206]:  # 支持206 Partial Content
            ...
            if response.headers.get("Content-Length"):
                response_headers["Content-Length"] = response.headers.get(
                    "Content-Length"
                )
            if response.headers.get("Content-Range"):
                response_headers["Content-Range"] = response.headers.get(
                    "Content-Range"
                )
            if response.headers.get("Accept-Ranges"):
                response_headers["Accept-Ranges"] = response.headers.get(
                    "Accept-Ranges"
                )
            if response.headers.get("ETag"):
                response_headers["ETag"] = response.headers.get("ETag")
            if response.headers.get("Last-Modified"):
                response_headers["Last-Modified"] = response.headers.get(
                    "Last-Modified"
                )
