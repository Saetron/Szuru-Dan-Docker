...
class Config:
    def __init__(self):
        ...
        # 检查是否启用反向代理模式
        self.REVERSE_PROXY_MODE = config.getboolean(
            "API", "reverse_proxy_mode", fallback=False
        )

        # 检查是否启用性能计时日志
        self.ENABLE_TIMING_LOGS = config.getboolean(
            "API", "enable_timing_logs", fallback=False
        )
